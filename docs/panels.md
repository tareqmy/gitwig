...
*   **Global Summary Header Bar**: Top row of four tabs showing aggregate counts: `repos │ dirty │ ahead │ stale`. Click a tab, or press `Tab` / `Shift+Tab`, to filter the repository list to that section; the active tab is drawn as a highlighted block and `Esc` clears the filter. When a label filter is active, a highlighted `● label` badge sits at the left of the tab strip (click it to reopen the label picker) and the tab counts scope to that label. If the active filters leave nothing to show, the tab strip stays visible above a message explaining which filter is empty and how to leave it.
