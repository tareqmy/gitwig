...
*   **Global Summary Header Bar**: Top row of four tabs showing aggregate counts: `repos │ dirty │ ahead │ stale`. Click a tab, or press `Tab` / `Shift+Tab`, to filter the repository list to that section; the active tab is drawn as a highlighted block and `Esc` clears the filter. When a label filter is active, a highlighted `● label` badge is set into the frame's top border, centered above the header (click it to reopen the label picker), and the tab counts scope to that label. If the active filters leave nothing to show, the tab strip stays visible above a message explaining which filter is empty and how to leave it.
*   **Quick Label Chips**: Once a label has been viewed through the label filter, a second row under the summary bar, set off by a thin dashed rule, lists up to nine labels as `1 web │ 2 api │ …` chips styled like the summary tabs. Slots fill FIFO: a label takes the next free slot the first time it is viewed and keeps that number (re-viewing never reorders), and once all nine are taken the oldest is evicted so the new label lands in slot 9. The list starts empty, is stored as `label_slots` in `state.toml` (the usage-state file beside `config.toml`), and drops labels no repository carries any more. Press the digit, or click the chip, to apply that label as the sticky filter; the active label's chip is drawn as a highlighted block, and pressing its digit again clears the filter.
*   **Sort Rule**: A thin dashed rule (`┄`, or `-` in compatibility mode) closes the header directly above the repository list and carries the active sort caption (`Sort: Custom`, `Sort: Alphabetical (Rev)`, …), which used to sit in the top frame border. When the active label filter sets its own sort in Label Settings, the caption ends with the label's name (`Sort: Alphabetical · work`) to show that the label's sort, not the global one, is in effect.
