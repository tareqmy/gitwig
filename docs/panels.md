...
*   **Sort Rule**: A thin dashed rule (`┄`, or `-` in compatibility mode) closes the header directly above the repository list and carries the active sort caption (`Sort: Custom`, `Sort: Alphabetical (Rev)`, …), which used to sit in the top frame border.
