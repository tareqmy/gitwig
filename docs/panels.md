...
*   **Global Summary Header Bar**: Top row of four tabs showing aggregate counts: `repos │ dirty │ ahead │ stale`. Click a tab, or press `Tab` / `Shift+Tab`, to filter the repository list to that section; the active tab is drawn as a highlighted block and `Esc` clears the filter. When a label filter is active, a `● label ▶` chip sits at the left of the tab strip (click it to reopen the label picker) and the tab counts scope to that label. If the active filters leave nothing to show, the tab strip stays visible above a message explaining which filter is empty and how to leave it.
*   **Quick Label Chips**: Once a label has been viewed through the label filter, a second row under the summary bar lists up to nine labels as `1 web  2 api …` chips. Slots fill FIFO: a label takes the next free slot the first time it is viewed and keeps that number (re-viewing never reorders), and once all nine are taken the oldest is evicted so the new label lands in slot 9. The list starts empty, is stored as `label_slots` in `config.toml`, and drops labels no repository carries any more. Press the digit, or click the chip, to apply that label as the sticky filter; the active label's chip is drawn as a highlighted block, and pressing its digit again clears the filter.
