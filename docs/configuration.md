...
| `enable_commit_signatures` | `Boolean` | `false` | Verify GPG/SSH signatures on the commits list and graph. Needs `gpg` (or `ssh-keygen` for SSH signatures) on `PATH`; when it is missing the graph still loads and the status bar reports it. |
