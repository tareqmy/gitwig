...
impl App {
    ...
    pub fn start_workspace_search(&mut self) {
        self.workspace_search_selection = 0;
        self.input_buffer.clear();
        self.previous_mode = Some(self.mode);
        self.mode = Mode::WorkspaceSearchInput;
    }

    pub fn get_workspace_search_matches(&self) -> Vec<(usize, usize, String, String)> {
        let mut results = Vec::new();
        if let Some(repo::ItemDetail::Repo { info, .. }) = &self.current_detail {
            let query = self.input_buffer.to_lowercase();
            let mut all_items = Vec::new();

            for (idx, item) in info.changes.staged.iter().enumerate() {
                all_items.push((0, idx, item.path.clone(), item.label.to_string()));
            }
            for (idx, item) in info.changes.unstaged.iter().enumerate() {
                all_items.push((1, idx, item.path.clone(), item.label.to_string()));
            }
            for (idx, item) in info.changes.untracked.iter().enumerate() {
                all_items.push((2, idx, item.path.clone(), item.label.to_string()));
            }
            for (idx, item) in info.changes.conflicted.iter().enumerate() {
                all_items.push((3, idx, item.path.clone(), item.label.to_string()));
            }
            ...
            let mut matches = Vec::new();
            for (bucket, idx, path, label) in all_items {
                let path_lower = path.to_lowercase();
                if path_lower.contains(&query) {
                    let score = 1000 - (path_lower.len() - query.len());
                    matches.push((score, bucket, idx, path, label));
                } else {
                    let mut path_chars = path_lower.chars();
                    let mut matched = true;
                    for qc in query.chars() {
                        if !path_chars.any(|nc| nc == qc) {
                            matched = false;
                            break;
                        }
                    }
                    if matched {
                        matches.push((100, bucket, idx, path, label));
                    }
                }
            }
            ...
            results = matches
                .into_iter()
                .map(|(_, bucket, idx, path, label)| (bucket, idx, path, label))
                .collect();
        }
        results
    }
