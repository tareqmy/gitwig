    ...
    let mut categories: Vec<(&str, Vec<(String, &str)>)> = vec![
        ...
        (
            ...
            vec![
                ...
                (
                    sort_key,
                    "Cycle sorting mode / Toggle reverse sorting (the active label's own sort when its Label Settings set one)",
                ),
                (
                    compact_key,
                ...
                (terminal_key, "Open embedded terminal panel in the selected repository"),
                (external_shell_key, "Suspend the TUI and open a full-screen shell"),
