...
resolve_version() {
    if [ "${VERSION:-}" = "" ]; then
        ...
        if [ -z "${LATEST_RELEASE:-}" ]; then
            error "Could not auto-detect latest version (this can happen due to network issues or GitHub API rate limits). Please specify the version manually, for example: VERSION=v2.3.4 ./install.sh"
        fi
