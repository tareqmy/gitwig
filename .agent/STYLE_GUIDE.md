...
- **Status bar:** Use a `MODE` badge with a colored background on the left and muted contextual text on the right, separated by `·`. Use the real terminal cursor (`Frame::set_cursor_position`) in input modes rather than a fake `_` character — it tracks blink and theme correctly.
