...
- **Item Statuses:** `App.statuses: Vec<ItemStatus>` runs parallel to `App.config.items`. Any mutation that adds, edits, or removes an item **must** update `statuses` at the same index in the same method (`commit_add` pushes, `commit_edit` overwrites at `selected_index`, `confirm_delete` removes). Drift between the two vectors causes wrong indicators after edits. Inspection is intentionally cheap (`std::fs` only — no `git2`) so it's safe to call per-mutation. If the inspection ever becomes slow (e.g., upgraded to a full `git2` open), introduce a background-refresh strategy rather than blocking on each keystroke.
