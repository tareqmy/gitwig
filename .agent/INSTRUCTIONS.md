...
- **Item Statuses:** `App.statuses: Vec<ItemStatus>` runs parallel to `App.config.items`. Any mutation that adds, edits, or removes an item **must** update `statuses` at the same index in the same method (`commit_add` pushes, `commit_edit` overwrites at `selected_index`, `confirm_delete` removes). Drift between the two vectors causes wrong indicators after edits. Inspection is intentionally cheap (`std::fs` only — no `git2`) so it's safe to call per-mutation. The richer `repo::inspect_detail` (git2-based) is **only** invoked from `App::open_detail` so the cheap list rendering doesn't pay for libgit2.
- **git2 API surprises in 0.21:** `Reference::shorthand` returns `Result<&str, Error>` (not `Option<&str>`); `Commit::summary` returns `Result<Option<&str>, Error>` (outer = read success, inner = UTF-8 validity); `StringArray::iter` yields `Result<Option<&str>, Error>`. Collapse with `.ok()` / `.ok().flatten()` / `let Ok(Some(name)) = name else continue;` — see `src/repo.rs` for the canonical patterns.
...
- `src/status.rs` — pure filesystem inspection: `ItemStatus` enum and `inspect(item)`. No UI awareness, no caching — `App` owns the cached statuses.
- `src/repo.rs` — heavier git2-based detail inspection used by the Detail view: `ItemDetail`, `RepoInfo`, `HeadInfo`, `RemoteInfo`, `WorktreeStatus`. Invoked **only on demand** via `App::open_detail`, never per frame.
- `src/ui_detail.rs` — rendering for the Detail view. Imports the theme constants/helpers from `src/ui.rs` (which were promoted to `pub(crate)`). Pure — no state.
