...
- **Item Statuses:** `App.statuses: Vec<ItemStatus>` runs parallel to `App.config.items`. Any mutation (add/edit/remove) **must** atomically update `statuses` at the same index in the same method to prevent visual drift. A status computed in the background must be stored with `App::store_status(idx, path, status)` (the index can be stale once the list re-sorts) and followed by `App::resort_after_status_refresh`, which re-sorts a Latest Changes list when a repository's last commit moved.
- **Home Cursor vs. Open Repository:** `App.selected_index` indexes the grouped home rows (`get_home_rows()`), never `config.items` / `statuses` / `get_filtered_items()`. Inside a repository view, identify the repository with `App::active_repo_item()`, never `get_selected_item()`; open a specific one with `App::open_repo(item)`; keep the cursor on a repository across a reorder with `home_cursor()` + `select_home_row()`. See `.agent/CODEMAP.md` §3 and the style guide.
- **Path-keyed Data:** `labels`, `repo_configs`, `pinned`, `starred` (config), `visits`, `commit_history` (state, the latter by resolved path) and `multi_selected` are keyed by a repository's path. Renaming an entry must go through `App::migrate_repo_path`, and a new path-keyed map must be added there, or an edit silently orphans it. Refuse a new or edited path that another entry already tracks (`is_repo_tracked` / `is_repo_tracked_except`).
- **Worktrees and Submodules by Path:** git identifies both by path. The `gitwig_core::worktree_lock` / `worktree_unlock` / `worktree_remove` helpers take `WorktreeInfo::path` — an admin name (`WorktreeInfo::name`) is resolved by git as a path suffix and can match a different worktree — and `worktree_remove` forces only when the user chose to. Submodule deletion goes through `gitwig_core::submodule_remove(repo, name, path)`: the path for `deinit` / `rm`, the name only for the module directory.
