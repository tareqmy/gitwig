...
- **Modal Input:** The app uses a `Mode` enum (`Normal`, `Adding`, `Editing`, `ConfirmDelete`, `Help`, `Detail`, `DetailOverview`, `DetailHelp`, `CommitInput`) defined in `src/app.rs` to interpret keystrokes. When adding a new keybinding: add the route in `src/input.rs` (`handle_key`), add a corresponding `App` method in `src/app.rs` if it mutates state, add an entry to the `HELP_LINES` constant in `src/ui.rs` (the source of truth for the `?` overlay), and update the status-bar text in `src/ui.rs::draw_status_bar` — all in the same change.
- **Detail-view focus and Mouse support:** When in `Mode::Detail` the focused sub-panel is tracked by `App.detail_focus: DetailSection`. `DetailSection` is an enum in `src/app.rs` with four variants: `Commits`, `Staged`, `Unstaged`, `StagingDetails`. `Tab` cycles focus. Focus can also be changed by left-clicking (mouse event) inside any panel's boundary, which hit-tests the recorded panel bounds stored in `App.detail_areas` (computed during the draw frame). In `Mode::Normal` (the main repository list), left-clicking an item card selects it (updates `App.selected_index`), and double-clicking (a second click on the same item within 400ms tracked by `App.last_click`) opens the detail view directly. Main item bounds are captured during drawing in `App.main_areas`. Focus always resets to `Commits` when `open_detail()` is called.
...
- `src/ui_detail.rs` — rendering for the Detail view. Imports the theme constants/helpers from `src/ui.rs` (which were promoted to `pub(crate)`). Pure — no state. Receives `&DetailSection` (the focused panel) and highlights it with an accent border. The view is structured as: a one-row breadcrumb header (item name left, branch name right), a top 50% `Commits` panel (rounded box), and a bottom 50% split into `Staging Area` (left, itself split into `Staged`/`Unstaged` sub-panels) and `Staging Details` (right).
