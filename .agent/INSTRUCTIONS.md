...
- **Detail-view focus:** When in `Mode::Detail` the focused sub-panel is tracked by `App.detail_focus: DetailSection`. `DetailSection` is an enum in `src/app.rs` with four variants: `Commits`, `Staged`, `Unstaged`, `StagingDetails`. `Tab` calls `App::cycle_detail_focus()` which advances through that cycle. Focus can also be changed by left-clicking (mouse event) inside any panel's boundary, which hit-tests the recorded panel bounds stored in `App.detail_areas` (computed during the draw frame). The focused panel is rendered with an accent-coloured border; all others use `muted_style()`. Focus always resets to `Commits` when `open_detail()` is called.
