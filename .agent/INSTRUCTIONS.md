...
- **Modal Input:** The app uses a `Mode` enum (`Normal`, `Adding`, `Editing`, `ConfirmDelete`, `Help`, `Detail`, `DetailOverview`, `DetailHelp`, `CommitInput`) defined in `src/app.rs` to interpret keystrokes. When adding a new keybinding: add the route in `src/input.rs` (`handle_key`), add a corresponding `App` method in `src/app.rs` if it mutates state, add an entry to the `HELP_LINES` constant in `src/ui.rs` or `DETAIL_HELP_LINES` in `src/ui_detail.rs` (the source of truth for overlays), and update the status-bar text in `src/ui.rs::draw_status_bar` — all in the same change.
- **Detail-view focus and Mouse support:** When in `Mode::Detail` the active tab is tracked by `App.detail_tab` (0 = Details, 1 = Graph, 2 = Branches, 3 = Files) and the focused panel/widget is tracked by `App.detail_focus: DetailSection`. `DetailSection` variants include `Commits`, `Staged`, `Unstaged`, `CommitDetails`, `StagingDetails`, `LocalBranches`, `RemoteBranches`, `Files`. Pressing `Tab` / `Shift+Tab` (`BackTab`) cycles between the tabs, updating the active panel to a default focus for the chosen tab (`Commits` for Details, `LocalBranches` for Branches, `Files` for Files). Pressing `w` / `W` cycles panel focus within the Details and Branches tabs. Branch panel selection also supports `Left` / `Right` arrow keys. Directories inside the Files tab support expansion (`Right` / `>` / `.`) and collapse (`Left` / `<` / `,`) and are stored in a `HashSet<String>` containing paths of expanded folders. Mouse left-clicks support switching active tabs by clicking tab headers and changing focus to clicked panels using the recorded panel bounds stored in `App.detail_areas` (computed during drawing). Double-clicking an item in `Mode::Normal` opens the detail view directly. Mouse wheel scroll is supported across all scrollable lists, graphs, and unified diffs.
