...
- [x] Keyboard panel resizing: `+`/`-` grow/shrink the focused Detail panel along the same splits the mouse drags.
...
- [x] Quick-Label Slots (`1`-`9`): up to nine labels as numbered chips under the summary bar, assigned FIFO on first view (a label keeps its slot; the oldest is evicted once all nine are taken) and persisted as `label_slots` in `state.toml`; the digit or a click applies that label's project view, the active slot again clears it, and the picker shows each slot's key.
...
- [x] Multi-select with `Space`: Select multiple repos and batch-operate (fetch, delete entries, open in external shell).
...
- [ ] **Snap/Flatpak/AppImage:** Provide universal Linux packages for distribution-agnostic installation without requiring root privileges.
