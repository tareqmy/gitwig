...
2. **Lint** — `make lint`, i.e. `cargo clippy --workspace --all-targets -- -D warnings -D clippy::unwrap_used`.
   ...
   that CI actually enforces. The `unwrap_used` deny is deliberate, not incidental: see the
   "No Panics in Hot Paths" rule in `.agent/STYLE_GUIDE.md` — `.unwrap()`/`.expect()` in
   rendering or the event loop are lint errors here, not style nits. Fix the underlying
   code; don't paper over it with `#[allow(...)]` unless a case is a genuine false positive.
