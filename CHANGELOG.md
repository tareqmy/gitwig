...
- **help**: key captions listed an alias twice when both spellings render the same, so the help overlay, command palette and status bar showed `Shift+Tab/Shift+Tab` (or `⇧⇥/⇧⇥`) for cycling tabs backward; each caption now appears once.
