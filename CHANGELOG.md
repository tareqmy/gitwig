...
## [v2.6.4] - 2026-09-25
### Fixed
- **worktrees**: removing a worktree (`D`) could force-delete a different worktree, uncommitted work and all. The worktree was named to `git worktree remove --force` by its admin name (the directory under `.git/worktrees`), which git resolves as the end of a worktree *path*: after a `git worktree move`, or when two worktrees share a folder name, that name matched another worktree or none. Removal, lock and unlock (`l`) now pass the worktree's checkout path, so they act on the row you selected — lock and unlock also stop failing with "is not a working tree" for such worktrees — and the worktree is captured when `D` is pressed, so a list reload while the prompt is open cannot change the target. ([af49f4b](https://github.com/tareqmy/gitwig/commit/af49f4b))
- **worktrees**: both choices of the remove prompt deleted the worktree folder with `--force`, so "1: Metadata only" discarded uncommitted changes too. The prompt now names the worktree and offers `1` — remove only if clean (git refuses a worktree with uncommitted changes or a lock, and nothing is deleted; the error says to choose `2`) — and `2` — force-remove, discarding uncommitted changes and overriding a lock. ([af49f4b](https://github.com/tareqmy/gitwig/commit/af49f4b))
- **submodules**: deleting a submodule (`D`) passed its *name* to `git submodule deinit` and `git rm`, which take a *path*. For a submodule whose name differs from its path (`git submodule add --name`, or a later `git mv`) the delete failed, and when a tracked file happened to share the name, that file was deleted along with its local edits while the submodule stayed registered and `git status` broke. The delete now uses the submodule's path, refuses anything that is not a submodule entry in the index, stops if `deinit` fails, and removes the module directory where git keeps it (inside a linked worktree too). The confirmation popup names the path, and the name when it differs. ([af49f4b](https://github.com/tareqmy/gitwig/commit/af49f4b))
- **forge**: the Issues and PRs tabs could stay on "Loading…" for good — after `R`, after checking out an issue's or a PR's branch (`Enter`), after a background refresh, or on reopening the repository. Finished tab loads only cleared their "loading" flag for tabs below index 10, and Issues and PRs are tabs 10 and 11, so they were never loaded again; a background refresh also dropped their lists. Both tabs now reload like the others, keep their lists across refreshes, and a load still running when you leave the repository no longer leaves its tab stuck when you come back. ([e8c6502](https://github.com/tareqmy/gitwig/commit/e8c6502))
- **forge**: the PR line-comment wizard (`n` in the PRs tab) could not be used. Its popup was never drawn (the home list was shown instead), typed keys were ignored, and three `Enter`s posted an empty comment on line 1. The popup now shows over the PRs tab, each step takes typing, Backspace and cursor keys, and an empty path or comment, or a line number that is not a whole number of 1 or more, shows an error and keeps the step open. The comment and file path are sent to `gh api` verbatim (`-f`), so a comment starting with `@name`, or one that is just a number, is posted as written instead of failing or being sent as a number. ([e8c6502](https://github.com/tareqmy/gitwig/commit/e8c6502))
- **forge**: the whole PR list failed to load ("missing field `name`") when any PR had a commit status — for example from an external CI — since those entries have a `context` instead of a `name`. They are now listed under CI/CD Checks by their context, and a check that is still running shows its status (`IN_PROGRESS`, `QUEUED`) instead of a blank. ([e8c6502](https://github.com/tareqmy/gitwig/commit/e8c6502))
- **forge**: `Enter` on an issue could check out the wrong branch. The branch linked to the issue was looked up with a `gh issue view` field that does not exist, so the lookup always fell through to picking the first branch whose name merely *contained* the number — issue #2 went to `issue-12-login`, issue #1 to `v1.2`. The linked branch is now read from GitHub, name matching only accepts the number as a token of its own, several candidates are reported instead of guessed between, and a linked branch missing from the clone is reported instead of being passed to `git checkout`, which would have read a name like `docs` as a path and discarded that folder's local changes. ([e8c6502](https://github.com/tareqmy/gitwig/commit/e8c6502))
- **forge**: when a PR's line comments failed to load (for example an HTTP 403, or a lapsed `gh` login), Gitwig ran `gh api` again on every frame — about ten times a second for as long as the PRs tab was open, eating into the GitHub API rate limit — and the details panel only ever showed "Loading line comments...". The failure is now kept and shown in the panel with `gh`'s message, and comments are requested again only when you select another PR, press `R`, or post a comment. ([928a131](https://github.com/tareqmy/gitwig/commit/928a131))
- **worktrees**: opening a worktree (`Enter` in the Worktrees tab) left the view half in the Advanced tab group: no tab was highlighted, `Shift+Tab` crashed debug builds ("attempt to subtract with overflow") and sent release builds to a blank tab, and it took two presses of `q` to get back. The worktree's new home-list row also read "missing" and was dropped by a sorted list, because it was added without a status, and a worktree already tracked under another spelling of its path was added a second time. Opening a repository now always starts in the Primary tabs with the Advanced tabs' selections reset, tab cycling can no longer underflow, and the worktree is added the way the home screen adds a repository — or its existing entry is opened. ([8bd9304](https://github.com/tareqmy/gitwig/commit/8bd9304))
- **forge**: a PR's line comments could appear under another PR. Selecting a PR with the mouse did not reload its comments, and a slow response for the previously selected PR could overwrite the current one's; comments also carried over to a PR with the same number in the next repository opened. Each comment load now records its PR and a result for any other PR is dropped, comments are shown only for the selected PR, and a selection changed by any means (keys, mouse, a list reload) loads that PR's comments. ([8bd9304](https://github.com/tareqmy/gitwig/commit/8bd9304))
- **advanced tabs**: checking out from the Reflog (`Enter` / `Space`), Issues or PRs (`Enter`) tab ran at once; it now asks first, like a commit checkout from the Workspace tab (`Enter` or `y` confirms, `n` / `Esc` cancels). ([addfd00](https://github.com/tareqmy/gitwig/commit/addfd00))
- **advanced tabs**: the Submodules and Reflog lists ignored mouse clicks and the wheel, and clicking a scrolled Reflog, Issues or PRs list selected the row at that height in an unscrolled list. Clicks now select the row they hit in all five Advanced lists, and the wheel moves the selection. The Worktrees and Submodules lists also scroll to keep the selection in view; rows past the panel's height used to be selectable but never drawn. ([addfd00](https://github.com/tareqmy/gitwig/commit/addfd00))
- **worktrees**: adding a worktree (`a`) could only check out an existing branch, and the prompt's own example, `main`, always failed because `main` is already checked out. A name that is not a branch, tag or commit now creates that branch for the new worktree (a branch that exists only on one remote is checked out tracking it), the prompt says so, and a folder path starting with `-` is no longer read as an option. ([addfd00](https://github.com/tareqmy/gitwig/commit/addfd00))
- **submodules**: after a delete (`D`) the submodule stayed listed as "Clean" until the removal was committed; it now reads "Removal staged". ([addfd00](https://github.com/tareqmy/gitwig/commit/addfd00))
- **detail**: `J` / `K` move the selection in the detail tabs, as `docs/panels.md` has always said and as the Logs and File History views already did. A `keybindings.toml` still holding the old default for these two bindings is upgraded to the new one; customised bindings are left alone. ([addfd00](https://github.com/tareqmy/gitwig/commit/addfd00))
- **forge**: only the first 30 line comments of a PR were loaded; all of them are now fetched. ([addfd00](https://github.com/tareqmy/gitwig/commit/addfd00))
- **help**: key captions listed an alias twice when both spellings render the same, so the help overlay, command palette and status bar showed `Shift+Tab/Shift+Tab` (or `⇧⇥/⇧⇥`) for cycling tabs backward; each caption now appears once. ([ed8329e](https://github.com/tareqmy/gitwig/commit/ed8329e))
- **home**: sorted by Latest Changes, the list went stale: its order was only computed when something else re-sorted it (changing the sort, adding a repository), so a repository that got a new commit — in Gitwig or outside it — kept its place until then. The list now re-sorts itself when a status refresh (the 10-second background refresh, the file watcher, a fetch, or returning from `git rebase -i` / `git mergetool`) finds a repository's latest commit has changed, and the cursor stays on the repository it was on. A refreshed status that arrived after the list had been re-sorted was also dropped, leaving that card out of date until the next refresh; it now reaches its repository. ([54fe0fc](https://github.com/tareqmy/gitwig/commit/54fe0fc))

### Chore
- **ci**: `make test`, which CI runs, was a plain `cargo test`, and at the workspace root that only tests the `gitwig` package: none of `gitwig-core`'s tests ran, in CI or locally. It now runs `cargo test --workspace`, and the contributor skills (`rust-quality`, `prepare-release`) say so. ([010d07a](https://github.com/tareqmy/gitwig/commit/010d07a))

### Documentation
- the Worktrees `D` and Submodules `D` rows in `docs/panels.md` describe the remove choices and what a submodule delete removes; the Issues `Enter` and PRs `n` rows describe how the issue's branch is chosen and the comment wizard's steps; the Worktrees `Enter` row says an already-tracked worktree is opened rather than added again; the Worktrees `a`, Submodules `D`, Reflog / Issues / PRs `Enter` rows, the confirmation lists in `docs/panels.md` and the detail `↑`/`↓` rows in `docs/keybindings.md` describe the changes above; `docs/detail_view.md` says Latest Changes re-sorts itself. ([af49f4b](https://github.com/tareqmy/gitwig/commit/af49f4b), [e8c6502](https://github.com/tareqmy/gitwig/commit/e8c6502), [8bd9304](https://github.com/tareqmy/gitwig/commit/8bd9304), [addfd00](https://github.com/tareqmy/gitwig/commit/addfd00), [54fe0fc](https://github.com/tareqmy/gitwig/commit/54fe0fc))
- `docs/detail_view.md` now documents the Submodules keys and gained Forge Issues and Forge PRs sections; its Worktrees section no longer describes a "metadata only" removal, and its navigation notes cover `J`/`K`, clicks on the Advanced lists and the Forge splitters. `docs/panels.md` no longer says the Issues tab shows PRs and CI status, and lists the worktree remove prompt and the PR comment wizard among the dialogs. `Esc` / `q` / `Q` are described as stepping back from the Advanced tabs before leaving the repository (`docs/panels.md`, `docs/keybindings.md`, `docs/detail_view.md`), and `docs/keybindings.md` / `docs/configuration.md` describe the keybinding upgrade for extended defaults. For contributors, `.agent/CODEMAP.md`, `.agent/INSTRUCTIONS.md`, `.agent/STYLE_GUIDE.md`, `GEMINI.md` and `.agent/ROADMAP.md` record the background-result drains, storing statuses by path, identifying worktrees and submodules by path, mapping clicks through a table's scroll offset, `ForgeCheckoutConfirm`, and `make test` covering both crates; the roadmap no longer claims the Submodules tab can initialize or update existing submodules. ([ed8329e](https://github.com/tareqmy/gitwig/commit/ed8329e))

## [v2.6.3] - 2026-09-24
### Fixed
- **detail**: the repository path at the top of the repository view could name a different repository from the one open — usually one you had visited recently. The header read the home list's cursor, and opening a repository records a visit that lifts it to the top of the Recent group, shifting the rows so the cursor landed on a neighbour. The header, the per-repository theme, the Repository Settings popup and the per-repository page size / max commits / resync settings now follow the repository that is actually open, and the home cursor is put back on the repository you opened so it is still selected when you return. Opening a worktree from the Worktrees tab no longer risks opening a different tracked repository, and a Global Search hit opens that repository even when a label filter hides it from the home list. ([fc1e13d](https://github.com/tareqmy/gitwig/commit/fc1e13d))
- **home**: with grouping on (Recent, Starred or label groups showing), changing the sort order or direction (`o` / `O`, or a label's own sort in Label Settings), pinning, adding a repository (one path or a whole directory), or editing a repository's path could leave the cursor on a different repository or on a group header: those actions set the cursor from the repository's position in the flat list instead of its row among the grouped rows. The cursor now stays on the repository, in the group it was selected from, and starring keeps it there too instead of jumping to the repository's first row under Recent or Starred. A newly added repository is selected even when its label group was collapsed (the group is expanded to show it), and a repository discovered in the background by a watched directory no longer shifts the cursor off the one you had selected. ([28d6c52](https://github.com/tareqmy/gitwig/commit/28d6c52))
- **repository actions**: with grouping on (Recent, Starred or label groups showing), or a search or label filter narrowing the home list, several actions ran against whichever tracked repository sat at the home cursor's row number in the unfiltered list, usually not the one you meant. In the Worktrees tab, unlocking a worktree (`l` on a locked one) and pruning stale worktree metadata (`p`) ran `git worktree` in that other repository instead of the open one; on the home screen, the external git app (`g`) and the full-screen shell (`T`, with nothing multi-selected) opened in its directory, even with a group header highlighted; and returning from an interactive rebase or `git mergetool` refreshed its home card instead of the open repository's. The Worktrees actions and the post-rebase / mergetool refresh now use the open repository, and the git app and shell open the highlighted repository (nothing, on a group header). ([de91a42](https://github.com/tareqmy/gitwig/commit/de91a42))
- **home**: editing a repository's path (`e`) lost its labels, star, per-repository settings (theme, page size, max commits, resync, auto-fetch interval, editor, note) and commit-message history, so it dropped into the Unlabeled group, and `config.toml` / `state.toml` kept orphaned entries under the old path. Only its pin and last visit time were carried over. Everything recorded under the old path now moves to the new one, including a multi-selection mark, so the repository stays in its label groups with the cursor on it. ([be33e68](https://github.com/tareqmy/gitwig/commit/be33e68))
- **home**: editing a repository's path (`e`) to a path another entry already tracks — as written, via `~`, or through a symlink — was accepted, leaving the same repository listed twice and overwriting the other entry's labels and per-repository settings with the edited one's. The edit is now refused with "Repository already added", as adding a repository already was, and nothing is changed. Editing a path to another spelling of the same repository's own path (`~/x` for its expanded form, say) still works. ([8156fa1](https://github.com/tareqmy/gitwig/commit/8156fa1))

### Documentation
- the `e`, `g`, `T` and `a` / `A` rows in `docs/keybindings.md` and `docs/panels.md`, the grouping note in `docs/panels.md`, the header and return-to-list notes in `docs/detail_view.md`, and the path-keyed settings in `docs/configuration.md` now describe the behaviour above; `.agent/CODEMAP.md` ("Home Cursor vs. Open Repository"), `.agent/INSTRUCTIONS.md` §2 and `GEMINI.md` record the home-cursor and path-keyed-data rules for contributors. ([a3e8baa](https://github.com/tareqmy/gitwig/commit/a3e8baa))

## [v2.6.2] - 2026-09-23
### Added
- **labels**: Label Settings gained a per-label sort order. Two new rows, **Sort By** (default / Custom / Alphabetical / Recent Visit / Latest Changes) and **Sort Reverse** (default / yes / no), are stored as `sort_by` and `sort_reverse` under `[label_configs.<label>]` and apply while that label's filter ("project view") is active: entering the view sorts the home list by the label's setting, leaving it restores the global order, and changing a row while the view is open re-sorts at once. The header's sort caption names the label whose sort is in effect (`Sort: Alphabetical · work`), and the home `o` / `O` keys cycle the label's value instead of the global one whenever the label defines it, so what you see is what the key changes. Labels that leave the rows at default inherit the global `sort_by` / `sort_reverse` as before. The home help overlay's sort row, the shipped `config/config.toml` sample and `docs/` describe the new rows. ([66dc689](https://github.com/tareqmy/gitwig/commit/66dc689), [729f009](https://github.com/tareqmy/gitwig/commit/729f009))
...
### Changed
- **tags**: the Tags tab now resolves its keys through the `[tags]` bindings (`checkout`, `delete`, `push`, `push_all`, `fetch`, `search`) instead of matching them literally, so rebinding them in `keybindings.toml` or Settings → Keybindings finally takes effect and the command palette's Tags entries run. Workspace commit checkout is `o` only: `O` was always captured by the Overview toggle, so the dead default was dropped. ([b61d3d3](https://github.com/tareqmy/gitwig/commit/b61d3d3))
- **hints**: the help overlays and the status bar now show the real keys for every configurable action, both cases included (`t/T`, `c/C`, `i/I`, `a/A`, …), and follow rebindings instead of hard-coded literals. The detail help merges the two `s` rows, gains the Files full-screen row, names advanced tab 4 "Issues", and shows `c/C` for branch create. The status bar gained entries for the label picker, the checkout-and-merge-into confirmation, the commit-message history picker, the stats dashboard and the PR line-comment steps, shows CONFIRM / INPUT / LABELS / DEBUG / LOGS / HISTORY / STATS badges where it used to fall back to NORMAL, shows Fetch `f/F` and Add Remote `a/A` on the Branches tab from either list, Discard `x/X` on the Files tab, `x/u` / `d/m` / `⌃K` / `⌃W` in the commit dialog, `Esc/q/O` and `s/S` in the Overview, the uppercase variants in the stash panel, `/` in the Logs view, `Esc/ctrl-p` and paging in the palette, both filter-cycle keys on the home screen, and "Sidebar" instead of "Back" while the Settings fields panel is focused. ([b61d3d3](https://github.com/tareqmy/gitwig/commit/b61d3d3))
- **config**: usage state that changes just by using the app — last-visit times (`visits`), per-repository commit-message history (`commit_history`), the quick-label slots (`label_slots`) and the sticky label filter (`active_label_filter`) — moved out of `config.toml` into a separate `~/.gitwig/state.toml` (beside whichever config file is in use), so opening a repository, committing, or viewing a label never rewrites hand-edited settings. Keys still found in an older `config.toml` are migrated into `state.toml` on the next launch, merged without overriding what it already holds, and `config.toml` is rewritten without them; an upgrade backs both files up as `.toml.bak`. Deliberate choices (`pinned`, `starred`, `labels`, `repo_configs`, `label_configs`) stay in `config.toml`. ([d9a526c](https://github.com/tareqmy/gitwig/commit/d9a526c))

### Documentation
- audit of every user-facing doc, sample and internal map against the code: `docs/keybindings.md` and `docs/panels.md` corrected for fetch `f/F`, `Q` as a close key, Settings `q`/`Esc` behaviour, Cherry-Pick confirm keys, `Enter`/`→` to open a repository, the sort-cycle names, File History paging, the Staged/Unstaged/Conflicts panel keys, missing paging rows, and a note on the views whose keys are still hard-coded; `docs/configuration.md` corrected defaults (`max_commits` 500, `compatibility_mode` true, `view_mode` "normal", `git_app` "gitui" with its allow-list, `editor` fallback chain, `scan.start_dir`), the first-run resolution order, the deprecated `scan.git_only`, and gained a per-repository settings section; the shipped `config/config.toml` and `config/keybindings.toml` samples now match the defaults and include the navigation, palette, checkout and mergetool bindings; `docs/features.md` and `docs/installation.md` (SSH command, Homebrew tap) corrected; the Settings page descriptions state the enforced limits and allowed values; `.agent/CODEMAP.md`, `INSTRUCTIONS.md` (renumbered sections, keybinding checklist now starts at `src/keybindings.rs`, realistic module-size rule, scoped git-subprocess rule), `STYLE_GUIDE.md`, `ROADMAP.md` (blame marked shipped, Phase 5 heading, key names) and `GEMINI.md` brought back in line with the tree; the `.agent/skills` mirrors resynced with `.claude/skills`. ([b61d3d3](https://github.com/tareqmy/gitwig/commit/b61d3d3))

### Fixed
- **config**: atomic saves use a per-process, per-write temporary file name so two writers aiming at the same file (a second Gitwig instance, parallel tests) can no longer rename each other's temp file away and fail the save. ([d9a526c](https://github.com/tareqmy/gitwig/commit/d9a526c))
...
### Documentation
- sync detail-view, features, roadmap, and changelog for panel resize ([dcdc444](https://github.com/tareqmy/gitwig/commit/dcdc444))
...
### Documentation
- sync configuration, changelog, and codemap for label filter work ([917605b](https://github.com/tareqmy/gitwig/commit/917605b))
